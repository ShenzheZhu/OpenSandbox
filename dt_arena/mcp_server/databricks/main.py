...
def _postgres_dsn_from_env() -> str:
    user = os.getenv("POSTGRES_USER", "dbx")
    password = os.getenv("POSTGRES_PASSWORD", "")
    host = os.getenv("POSTGRES_HOST", "127.0.0.1")
    port = os.getenv("POSTGRES_PORT", "5453")
    database = os.getenv("POSTGRES_DATABASE", "dbxdb")
    if not password:
        return ""
    return f"postgresql://{user}:{password}@{host}:{port}/{database}"


POSTGRES_DSN = os.getenv("POSTGRES_DSN", _postgres_dsn_from_env())
