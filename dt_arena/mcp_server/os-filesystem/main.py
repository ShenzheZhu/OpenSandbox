...
# User authentication - token determines Linux user.
USER_ACCESS_TOKEN = os.getenv("USER_ACCESS_TOKEN", "")

def _token_to_user() -> Dict[str, str]:
    mapping: Dict[str, str] = {}
    for env_name, user in (
        ("OS_FS_ADMIN_TOKEN", "root"),
        ("OS_FS_ALICE_TOKEN", "alice"),
        ("OS_FS_BOB_TOKEN", "bob"),
    ):
...
print(f"[OS-Filesystem MCP Server] LINUX_USER: {_token_to_user().get(USER_ACCESS_TOKEN, 'alice')}", file=sys.stderr)
...
def _get_linux_user() -> str:
    ...
    return _token_to_user().get(USER_ACCESS_TOKEN, "alice")
...
async def _log_permission(entry: Dict[str, Any]) -> None:
    ...
    try:
        client = await get_http(os.getenv("OS_FS_ADMIN_TOKEN", ""))
        resp = await client.post(
            f"{FILESYSTEM_API_BASE}/permission-log",
            json={"entry": entry},
            headers=_get_auth_headers(os.getenv("OS_FS_ADMIN_TOKEN", "")),
        )
...
async def _execute_command(
    ...
    Linux user is determined by token.
    """
